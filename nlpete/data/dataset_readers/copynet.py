...
class CopyNetDatasetReader(DatasetReader):
    ...
    def text_to_instance(
        self, source_string: str, acoustic_data: dict, target_string: str = None
        ...
        *including* the `START_SYMBOL` and `END_SYMBOL`.
        ...
        for i in tokenized_source:
            if i.text in acoustic_data:
                acoustic_score.append(acoustic_data[i.text])
            else:
                acoustic_score.append(0.0)
        source_field = TextField(tokenized_source, self._source_token_indexers)
        acoustic_data = acoustic_score
        ...
        if target_string is not None:
            ...
            fields_dict["acoustic_data"] = ArrayField(np.array(acoustic_data))
