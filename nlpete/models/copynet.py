...
class CopyNet(Model):
    ...
    def __init__(
        ...
        warnings.warn(
            "The 'copynet' model has been deprecated in favor of "
            "the 'copynet_seq2seq' model (now part of the AllenNLP library).",
            DeprecationWarning,
        )
    ...
    def forward(
        ...
        if target_tokens:
            ...
            output_dict = self._forward_loss(
                target_tokens, target_token_ids, acoustic_data, state)
        else:
            output_dict = {}
    ...
    def _get_ll_contrib(
        ...
        # Introducing the acoustic_data acoustic prominence scores 
        # copy_log_probs = copy_log_probs.add(acoustic_data)
    ...
    def _forward_beam_search(
        ...
        acoustic_data = acoustic_data[:,1:-1]
        # acoustic_data = acoustic_data * torch.norm(state["copy_log_probs"]) / torch.norm(acoustic_data)
        state["acoustic_data"] = acoustic_data
        # if (not torch.isnan(acoustic_data).any()):
        #     state["copy_log_probs"] = 0.7*state["copy_log_probs"] + 0.3*acoustic_data
        # else
    ...
    def take_search_step(
        ...
        # shape: (group_size, trimmed_source_length)
        copy_scores = self._get_copy_scores(state)
        state["acoustic_data"] = state["acoustic_data"] * torch.norm(state["copy_log_probs"]) / torch.norm(state["acoustic_data"])
        if (not torch.isnan(state["acoustic_data"]).any()):
            copy_scores = copy_scores + 0.005*state["acoustic_data"]
